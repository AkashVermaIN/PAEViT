...
def dataload(args, augmentations, normalize, data_info):
    if args.dataset == 'CIFAR10':
        ...
        train_dataset = datasets.SVHN(
            root=os.path.join(args.data_path,'svhn'), split='train', download=True, transform=augmentations)
        val_dataset = datasets.SVHN(
            root=os.path.join(args.data_path,'svhn'), split='test', download=True, transform=transforms.Compose([
            transforms.Resize(data_info['img_size']),
            transforms.ToTensor(),
            *normalize]))
        ...
        train_dataset = datasets.ImageFolder(
            root=os.path.join(args.data_path, 'tiny-imagenet-200', 'train'), download=True, transform=augmentations)
        val_dataset = datasets.ImageFolder(
            root=os.path.join(args.data_path, 'tiny-imagenet-200', 'val'), download=True, 
