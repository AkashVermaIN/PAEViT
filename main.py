...
def init_parser():
    ...
    parser.add_argument('--dataset', default='CIFAR10', choices=['CIFAR10', 'CIFAR100', 'T-IMNET'], type=str, help='Dataset Path')
...
def main(args):
    ...
    logger.debug(f'Initial learning rate1: {args.lr:.6f}')
    logger.debug(f"Started training for {args.epochs} epochs")
    ...
    for epoch in tqdm(range(start_epoch, args.epochs)):
        ...
        current_accuracy = validate(val_loader, model, criterion, lr, args, epoch=epoch)
        torch.save({
        ...
        print(f'Best Accuracy now is {best_accuracy:.2f}')        
        writer.add_scalar("Learning Rate", lr, epoch)
        
    logger.debug(f'Best Top-1: {best_accuracy:.2f}, Final Top-1: {current_accuracy:.2f}')
...
def train(train_loader, model, criterion, optimizer, epoch, scheduler, args):
    model.train()
    loss_val, current_accuracy_val = 0, 0
    n = 0
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        current_accuracy = acc[0]
        n += images.size(0)
        loss_val += float(loss.item() * images.size(0))
        current_accuracy_val += float(current_accuracy[0] * images.size(0))
        ...
        if args.print_freq >= 0 and i % args.print_freq == 0:
            avg_loss, avg_current_accuracy = (loss_val / n), (current_accuracy_val / n)
            progress_bar(i, len(train_loader),f'[Epoch {epoch+1}/{args.epochs}][T][{i}]   Loss: {avg_loss:.4e}   Top-1: {avg_current_accuracy:6.2f}   LR: {lr:.7f} '+' '*10)

    logger_dict.update(keys[0], avg_loss)
    logger_dict.update(keys[1], avg_current_accuracy)
    writer.add_scalar("Loss/train", avg_loss, epoch)
    writer.add_scalar("Acc/train", avg_current_accuracy, epoch)
    
    f = open(save_path+'/results.txt', 'a')
    f.write('Train | Epoch: %d | Loss: %.3f | Acc: %.3f\n'
                % (epoch+1, avg_loss, avg_current_accuracy))
    f.close()
...
def validate(val_loader, model, criterion, lr, args, epoch=None):
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            if args.print_freq >= 0 and i % args.print_freq == 0:
                avg_loss, avg_current_accuracy = (loss_val / n), (current_accuracy_val / n)
                progress_bar(i, len(val_loader), f'[Epoch {epoch+1}][V][{i}]   Loss: {avg_loss:.4e}   Top-1: {avg_current_accuracy:6.2f}   LR: {lr:.6f}')
    print()        
    
    logger_dict.update(keys[2], avg_loss)
    logger_dict.update(keys[3], avg_current_accuracy)
    
    writer.add_scalar("Loss/val", avg_loss, epoch)
    writer.add_scalar("Acc/val", avg_current_accuracy, epoch)
