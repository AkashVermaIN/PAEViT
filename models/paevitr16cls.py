...
class Attention(nn.Module):
    def __init__(self, dim, num_patches, heads = 8, dim_head = 64, dropout = 0.):
        ...
        self.excitation = nn.Sequential(
            nn.Linear(self.num_patches+1, 16),
            nn.ReLU(),
            nn.Linear(16, self.num_patches+1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        ex = torch.mean(dots,dim=3)
        ex = torch.mean(ex,dim=1)

        ex = self.excitation(ex)

        ex = rearrange(ex, 'b (h d p) -> b h d p', h = 1, p = 1)

        ex = ex.repeat(1,self.heads,1,self.num_patches+1)

        attn = ex * self.attend(dots) # e, f
